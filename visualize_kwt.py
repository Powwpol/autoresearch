...
# ── couleurs charte neutre (pas Pulsa sur output public RunPod) ──────────────
C_BASE = "#4c72b0"   # bleu steel
C_KWT  = "#dd8452"   # orange
C_SCALE = "#55a868"  # vert
...
def make_static_plot(results: dict, out_dir: str) -> str:
    ...
    for label, run, color in [("Baseline", base, C_BASE), ("Baseline+KWT", kwt, C_KWT)]:
        ...
        if curve:
            xs = curve["steps"]
            ys = curve["loss_proxy"]
            ax_curve.plot(xs, ys, label=label, color=color, linewidth=1.8)
    ...
    if vals:
        bars = ax_bar.bar(labels, vals, color=colors, width=0.4, edgecolor="white")
        ax_bar.set_ylabel("val_bpb final (bits/byte, ↓ mieux)")
        ax_bar.set_title("val_bpb final")
        ymin = min(vals) * 0.995
        ymax = max(vals) * 1.005
        ...
        if delta is not None:
            ...
            ax_bar.set_title(f"val_bpb final  (Δ KWT-Base = {sign}{delta:.6f})")
...
def make_video(results: dict, out_dir: str) -> str:
    ...
    ax.set_title("KWT vs Baseline — val_bpb proxy (5 min H100)", color="#e6edf3", pad=10)
    ax.set_xlabel("Pas d'entraînement")
    ax.set_ylabel("val_bpb proxy (loss EMA)")

    all_ys = ys_b + ys_k
    y_min, y_max = min(all_ys) * 0.98, max(all_ys) * 1.02
    x_max = max(max(xs_b or [1]), max(xs_k or [1]))
    ax.set_xlim(0, x_max * 1.02)
    ax.set_ylim(y_min, y_max)
    ax.grid(True, alpha=0.15, color="#444c56")

    line_b, = ax.plot([], [], color=C_BASE, linewidth=2, label="Baseline")
    line_k, = ax.plot([], [], color=C_KWT, linewidth=2, label="Baseline+KWT")
    txt = ax.text(0.02, 0.95, "", transform=ax.transAxes, color="#e6edf3",
                  fontsize=10, va="top")
    ax.legend(facecolor="#161b22", edgecolor="#444c56", labelcolor="#e6edf3")
    ...
    anim.save(str(out), writer=animation.FFMpegWriter(fps=12, bitrate=2400))
    plt.close(fig)
...
def make_kwt_scale_plot(trace: list[dict], out_dir: str) -> str:
    ...
    ax1.fill_between(steps, 1.0, scales, alpha=0.3, color=C_KWT)
    ax1.plot(steps, scales, color=C_KWT, linewidth=1.5, label="kwt_scale = 1+f*")
    ax1.axhline(1.0, color="#888", linestyle="--", linewidth=0.8, label="baseline (1.0)")
    ax1.set_ylabel("LR scale factor")
    ax1.set_ylim(0.95, max(scales) * 1.05)
    ax1.legend(loc="upper right", fontsize=9)
    ax1.grid(True, alpha=0.3)

    ax2.fill_between(steps, 0, fstars, alpha=0.3, color=C_SCALE)
    ax2.plot(steps, fstars, color=C_SCALE, linewidth=1.5, label="f* (Kelly fraction)")
